...
namespace Project6_ApiWeather.Controllers
{
	...
	public class WeathersController : ControllerBase
	{
		...
		// [HttpGet] -> API projelerinde her metodun attribute'ı olmalı. Yoksa hata verir.
		[HttpGet]
		...
		{
			var values = context.Cities.OrderBy(x => x.Temp).Select(y => y.CityName).FirstOrDefault(); // order by asc
			return Ok(values);
		}
	}
}
